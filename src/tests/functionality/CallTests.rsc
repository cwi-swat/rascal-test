...
public test bool keywordTest34(){
  ...
  data Volume = volume(int width, int height, int depth, int surface = width * height, int volume = surface * depth);
  
  test bool dynamicKeywords1() 
    = v := volume(2,3,4) && v.surface == v.width * v.height && v.surface * v.depth == v.volume;
    
  test bool dynamicKeywords2()
    = volume(_,_,_,volume=v) := volume(2,3,4) && v == 2 * 3 * 4;
    
  test bool dynamicKeywords3()
    = volume(_,_,_,volume=v) := volume(2,3,4, surface=9) && v == 9 * 4;
